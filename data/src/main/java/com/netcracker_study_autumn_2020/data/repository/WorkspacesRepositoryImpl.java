...
public class WorkspacesRepositoryImpl implements WorkspaceRepository {
    ...
    @Override
    public void allWorkspaces(long userId, String accessType, WorkspaceListCallback callback) {
        WorkspaceEntityStore workspaceEntityStore = workspaceEntityStoreFactory.create();
        workspaceEntityStore.allWorkspaces(userId, SpaceAccessType.valueOf(accessType), new WorkspaceEntityStore.WorkspaceListCallback() {
            @Override
